...
module.exports = {
  createAssignment: (data, callBack) => {
    pool.query(
      'insert into assignment(naam, start_datum, inlever_datum, vak, punten, herkansingspunten, roadmap_id) values(?,?,?,?,?,?,?)',
      [
          data.naam,
          data.start_datum,
          data.inlever_datum,
  ...
  updateAssignment: (data, id, callBack) => {
    pool.query(
      'update assignment set naam = ?, start_datum = ?, inlever_datum = ?, vak = ?, punten = ?, herkansingspunten = ?, roadmap_id = ? where id = ?',
      [
        data.naam,
        data.start_datum,
        data.inlever_datum,
